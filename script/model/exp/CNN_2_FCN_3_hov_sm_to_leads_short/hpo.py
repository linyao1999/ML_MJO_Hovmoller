...
base_config["data"]["train_start"] = '2000-01-01'
...
def objective(trial):
    ...
    if config["model"]["name"] == "CNN_MLP": 
        ...
        for i in range(config["model"]["cnn"]["layer_num"]):
            # channel = trial.suggest_categorical(f"channel_{i+1}", [4, 8, 16])
            channel = 16
            channels_list.append(channel)
        config["model"]["cnn"]["channels_list"] = channels_list
    elif config["model"]["name"] == "UNet_A":
        num_filters_enc = trial.suggest_categorical("num_filters_enc", [1, 2, 4, 8])
    ...
    if config["dataset_type"][:3] == "map" :
        ...
        kernel_size_ratio = 2
        # kernel_size_ratio = trial.suggest_categorical("kernel_size_ratio", [8, 4, 2])
    ...
    if config["model"]["mlp"]["layer_num"] == 0:
        ...
        #     hidden_layer = trial.suggest_categorical(f"hidden_layer_{i+1}", [64, 128, 256, 512])
    ...
    # ------------- training ----------------
    config["training"]["learning_rate"] = trial.suggest_float("learning_rate", 1e-4, 5e-3, log=True)
    config["training"]["batch_size"] = trial.suggest_categorical("batch_size", [16, 32])
    config["model"]["mlp"]["dropout"] = trial.suggest_float("fc_dropout", 0.1, 0.5, step=0.05)
