...
# ======================================================================
window_len = int(os.environ.get('window_len', 11))
residual = os.environ.get('residual', 'false').lower() == 'true'

with open(f'./yaml/hpo_sm{window_len}_res{str(residual)}.yaml', "r") as f:
...
config["model_save_path"] = config["model_save_path"].format(
    dataset_type=config["dataset_type"]+f'{config["data"]["window_len"]}_res{str(base_config["data"]["residual"])}',
...
with open(f'./yaml/best_config_sm{window_len}_res{str(residual)}.yaml', 'w') as f:
    yaml.dump(config, f)
