...
base_config["model"]["name"] = "MLP"  # "CNN_MLP", "UNet_A", "MLP"
...
def objective(trial):
    ...
    if config["dataset_type"][:3] == "map" :
        ...
        if config["model"]["name"] == "MLP":
            kernel_size_h, kernel_size_w = 0, 0
        else:
    ...
    # ------------- kernel size for the CNN ----------------
    if config["model"]["name"] == "MLP":
        config["model"]["cnn"]["kernel_size"] = [int(0), int(0)]
        config["kernel_size_str"] = f"0_0"
    else:
    ...
    if model_name == "CNN_MLP":
        ...
        flattened_size = config["model"]["cnn"]["input_channel_num"] * config["model"]["cnn"]["input_map_size"]
        model = MLP(
            input_size=flattened_size,
            hidden_layers=config["model"]["mlp"]["hidden_layers"],
            output_size=config["model"]["mlp"]["output_size"],
            dropout=config["model"]["mlp"].get("dropout", 0.5)
        )
    else:
